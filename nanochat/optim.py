...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def _step_muon(self, group: dict) -> None:
        ...
        if not params:
            return
        active_entries = [(i, p) for i, p in enumerate(params) if p.grad is not None]
        if not active_entries:
            return
        active_indices = torch.tensor([i for i, _ in active_entries], device=params[0].device, dtype=torch.long)
        active_params = [p for _, p in active_entries]
        ...
        # Stack only params that participated in this graph. Conditional modules
        # such as ACT/DSA can leave some matrix weights with grad=None.
        stacked_grads = torch.stack([p.grad for p in active_params])
        stacked_params = torch.stack(active_params)
        active_momentum_buffer = momentum_buffer.index_select(0, active_indices)
        active_second_momentum_buffer = second_momentum_buffer.index_select(0, active_indices)
        ...
        muon_step_fused(
            stacked_grads,
            stacked_params,
            active_momentum_buffer,
            active_second_momentum_buffer,
        ...
        momentum_buffer.index_copy_(0, active_indices, active_momentum_buffer)
        second_momentum_buffer.index_copy_(0, active_indices, active_second_momentum_buffer)
